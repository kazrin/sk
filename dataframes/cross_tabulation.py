...
class ShisetsuKijunFilingCrossTabDataFrame(pd.DataFrame):
    ...
    def from_jaccard_similarity(cls, jaccard_df, source_df, target_institution_name, top_n=20, target_institution_number=None):
        ...
        institution_filings_by_number = (
            source_df.groupby("医療機関番号")["受理届出名称"].apply(lambda x: set(x.dropna().unique())).to_dict()
        )

        # Get institution numbers for these institutions
        institution_number_mapping = source_df.groupby("医療機関名称")["医療機関番号"].first().to_dict()

        # Create mapping from 受理届出名称 to 受理記号 (1-to-1 relationship)
        if "受理記号" in source_df.columns:
            filing_name_to_symbol = source_df.groupby("受理届出名称")["受理記号"].first().to_dict()
        else:
        ...
        for filing_type in all_filing_types:
            row_data = {"受理届出名称": filing_type, "受理記号": filing_name_to_symbol.get(filing_type, "")}
    ...
    def get_display_dataframe(self, target_institution_name):
        ...
        institution_columns = [col for col in display_df.columns if col not in ["受理届出名称", "受理記号"]]
